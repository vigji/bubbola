...
import pandas as pd
from skrub import Joiner, deduplicate
...
def parse_items(xml_file):
    ...
    for det in root.findall(".//{*}DettaglioLinee"):
        entry = {child.tag.split("}", 1)[-1]: child.text for child in det}
        items.append(entry)
    return items
...
def parse_items_df(results_dir):
    ...
    items_df["ddt_delivery_date"] = items_df["ddt_delivery_date"].apply(
        lambda x: pd.to_datetime(x, format="%d/%m/%Y")
    )
    items_df["item_name_deduplicated"] = deduplicate(
        items_df["item_name"], n_clusters=30
    ).values
    return items_df


def parse_correct_items(results_dir):
    correct_items = pd.read_csv(
        results_dir.parent.parent / "fatture" / "invoice_items.csv"
    )
    correct_items = correct_items.rename(
        {
            "Description": "item_name",
            "Quantity": "item_quantity",
            "DDTDate": "ddt_delivery_date",
    ...
    correct_items["ddt_delivery_date"] = correct_items["ddt_delivery_date"].apply(
        lambda x: pd.to_datetime(x, format="%d/%m/%y")
    )
...
def parse_correct_xml(xml_file):
    ...
    items_df = items_df.rename(
        {
            "Descrizione": "item_name",
            "Quantita": "item_quantity",
            "PrezzoUnitario": "item_unit_price",
            "PrezzoTotale": "item_total_price",
        },
        axis=1,
    )
    for col in ["item_quantity", "item_unit_price", "item_total_price"]:
...
# analysis for 1461
results_dir = Path(
    "/Users/vigji/Desktop/pages_sample-data/concrete/1461/results/results_o4-mini_20250706_192000"
)
